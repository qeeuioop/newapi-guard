...
对所有经过 `/v1/*` 的 API 请求进行透明代理，在转发到 New API 之前执行用户级 RPM 限速；对实际调用类请求（POST 等非 GET）额外执行 UA 白名单检查。GET 常用于模型列表等读取类请求，当前设计保持宽松，不计入 UA 违规，避免误封公益站用户。
...
请求进入 Guard /v1/*
    │
    ├─ HTTP 方法是 OPTIONS？
    │   └─ 是 → 直接透传到 New API，流程结束
    │
    ├─ HTTP 方法是 GET？
    │   └─ 是 → 解析 token / 封禁 / RPM 后透传；不做 UA 违规计数，避免模型列表请求误封
    │
    └─ 是 POST 等实际调用 → 进入完整检查流程
         │
         │  ① 提取 Token
         │  依次检查以下位置，取到即停：
...
### 6.4 Anthropic Prompt Cache 注入

当后台 `prompt_cache_enabled=true` 时，Guard 会对 Anthropic Messages 请求自动注入 5 分钟 `cache_control`：

- `POST /v1/messages`：在通过 token、封禁、白名单、UA、RPM 检查后，Guard 会在转发给 New API 前改写 JSON body。
- `/upstream/anthropic/*`：如果该 Guard upstream 入口被使用，也复用同一套注入逻辑。

当前只使用默认 5 分钟 TTL，即 `{"type":"ephemeral"}`，不写 `ttl:"1h"`。因此 NewAPI 价格不需要改为 1 小时缓存写入价；仍按 5 分钟缓存写入约 `1.25x` input、缓存读取约 `0.1x` input 维护。

注入位置遵循 prefix-cache 原则：优先标记稳定前缀。`system` 存在时标记 system 最后一个 block；`messages` 有两条以上时优先标记倒数第二条 message 的最后一个 content block，避免把当前最后一条每轮变化的 user 消息作为主要缓存点。开启 `prompt_cache_debug=true` 时，Guard 只输出路径、模型、是否注入、message index 等结构化日志，不输出用户 prompt 内容。

### 6.5 签到拦截

对 `POST /api/user/checkin` 的请求，Guard 会先执行余额阈值判断。当前代码的实际行为是：余额达到 `checkin_threshold` 时拒绝签到；余额低于阈值或阈值为 0 时，直接透传到 New API 原生签到实现。Guard 当前不自行发放固定额度，也不写入本地 `checkin_records` 作为主日志来源。
...
请求进入 Guard /api/user/checkin
    ...
    │
    │  ③ 透传签到
    │  quota < threshold 或 threshold = 0 → 直接透传到 New API 原生签到实现
    │  成功响应与签到记录由 New API 负责
```
...
- Guard 当前保持 New API 原生签到结构与记录来源，日志页通过 NewAPI PostgreSQL `checkins` 表读取签到日志
