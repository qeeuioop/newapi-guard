...
func (h *Handler) reconcileUsers(ctx context.Context, adminToken string) map[int64]int64 {
	...
	if err == nil {
		for linkRows.Next() {
			...
			if user, ok := remoteByUsername[preferredUsername.String]; ok {
				_, _ = h.db.Exec(`INSERT INTO users(newapi_user_id, username, display_name, discord_id, discord_name)
					...
					ON CONFLICT(newapi_user_id) DO UPDATE SET
						...
						discord_name=CASE WHEN excluded.discord_name IS NOT NULL THEN excluded.discord_name ELSE users.discord_name END`, user.ID, user.Username, user.DisplayName, discordID.String, discordName.String)
				_, _ = h.db.Exec(`UPDATE oauth_identity_links SET newapi_user_id=?, updated_at=CURRENT_TIMESTAMP WHERE discord_id=?`, user.ID, discordID.String)
			}
		}
		_ = linkRows.Close()
	}
...
func (h *Handler) mergeLocalUserIDs(fromID, toID int64, username, displayName, discordID, discordName string) error {
	...
	if _, err := h.db.Exec(`INSERT INTO users(newapi_user_id, username, display_name, discord_id, discord_name)
		VALUES(?, ?, ?, NULLIF(?, ''), NULLIF(?, ''))
		ON CONFLICT(newapi_user_id) DO UPDATE SET
			username=CASE WHEN excluded.username != '' THEN excluded.username ELSE users.username END,
			display_name=CASE WHEN excluded.display_name != '' THEN excluded.display_name ELSE users.display_name END,
	...
	_, _ = h.db.Exec(`UPDATE bans SET newapi_user_id=? WHERE newapi_user_id=?`, toID, fromID)
	_, _ = h.db.Exec(`UPDATE token_cache SET newapi_user_id=? WHERE newapi_user_id=?`, toID, fromID)
	_, _ = h.db.Exec(`DELETE FROM ua_strikes WHERE newapi_user_id=?`, fromID)
	_, _ = h.db.Exec(`UPDATE bans SET unbanned_at=CURRENT_TIMESTAMP WHERE newapi_user_id=? AND unbanned_at IS NULL AND id NOT IN (
		SELECT id FROM bans WHERE newapi_user_id=? AND unbanned_at IS NULL ORDER BY created_at DESC LIMIT 1
	)`, toID, toID)
	_, _ = h.db.Exec(`DELETE FROM users WHERE newapi_user_id=?`, fromID)
	return nil
}
